...
<div class="col-md-12 my-5">
    ...
    <div class="card card-primary">
        ...
        <!-- /.card-header -->
        <!-- form start -->
        <form action="{{ route('admindoctor-books.update', $book->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="card-body">
                <div class="form-group">
                    <label for="doctor_name">Doctor Name</label>
                    <input type="text" class="form-control" id="doctor_name" value="{{ $book->doctor->name }}" readonly>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="name" name="name" value="{{ old('name', $book->name) }}" placeholder="Enter Patient Name" required>
                    @error('name')
                ...
                <div class="form-group">
                    ...
                    <input type="email" class="form-control" id="email" name="email" value="{{ old('email', $book->email) }}" placeholder="Enter User Email" required>
                    @error('email')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="tel" class="form-control" id="phone" name="phone" value="{{ old('phone', $book->phone) }}" placeholder="Enter User Phone" required>
                    @error('phone')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
